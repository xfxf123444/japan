...
InstallOptions::initDialog /NOUNLOAD "$PLUGINSDIR\finisha.ini"
  ...
  Pop $0
      ...
      ReadINIStr $INI1 "$PLUGINSDIR\finisha.ini" "Field 3" "HWND"
      SetCtlColors $INI1 0x000000 0xFFFFFF
      CreateFont $ZT1 "Tahoma" "11" "700"
      SendMessage $INI1 ${WM_SETTEXT} 0 "STR:$(^NameDA) セットアップ $\nウィザードは完了しました。"
      SendMessage $INI1 ${WM_SETFONT} $ZT1 0

      ReadINIStr $INI1 "$PLUGINSDIR\finisha.ini" "Field 2" "HWND"
      SetCtlColors $INI1 0x000000 0xFFFFFF
      SendMessage $INI1 ${WM_SETTEXT} 0 "STR:$(^NameDA)は、このコンピュータ$\nにインストールされました。$\r$\n$\r$\nウィザードを閉じるには [完了] を押してください。"

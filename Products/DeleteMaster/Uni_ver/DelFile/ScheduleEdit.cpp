...
void CScheduleEdit::OnModifySchedule() 
{
	...
	for (i = 0;i < nTotal;i ++)
	{
		...
		{
			int nItemIndex;
			ulItemNum = m_ScheduleList.GetItemData(i);

			for( nItemIndex=0; nItemIndex<g_ScheduleItemArray.GetSize(); nItemIndex++ )
			{
				if(g_ScheduleItemArray[nItemIndex].ulItemNum == ulItemNum)
				{
...
void CScheduleEdit::ModifyListItem(int nItem,PSCHEDULE_ITEM pScheduleInfo,BOOL bInsert)
{
	...
	m_ItemNum ++;
	pScheduleInfo->ulItemNum = m_ItemNum;
	...
	m_ScheduleList.SetItemData(nItem,m_ItemNum);

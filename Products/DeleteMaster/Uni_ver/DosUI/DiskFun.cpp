...
BOOL SaveReport(char csDrive)
{
	...
	szInfo[0] = csDrive;
 	...
 	if(g_btCurFun == DELETEPARTITION)
 	{
 		...
 		sprintf(szInfo,DELPARREPORT,szdriveletter,
 				g_pDiskInfo->btDiskNum+1, dwStart, dwEnd);
 		btDisk = g_pDiskInfo->btDiskNum;
 	}
 	else
 	{
 		sprintf(szInfo,DELDISKREPORT,g_deldiskinfo.nSelDisk-DISKBASE+1,
 				g_deldiskinfo.dwCylinders,g_deldiskinfo.dwHeads,g_deldiskinfo.dwSecPerTrack);
 		btDisk = g_deldiskinfo.nSelDisk-DISKBASE;
 	}
 	//get disk indentify
...
BOOL SaveVerifyLog(char csDrive,DWORD dwStartSec,DWORD dwSectors,BOOL bWriteFail)
{
	...
	if (bWriteFail)
		sprintf(szInfo,"Write start sector:%lu sectors:%lu failed",dwStartSec,dwSectors);
    else

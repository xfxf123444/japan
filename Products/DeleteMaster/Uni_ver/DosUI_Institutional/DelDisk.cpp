...
char showdeldiskwin()
{
	...
	char				*str[] = {"[V]iew Disk","Delete [M]ethod" ,"[I]dentify","[D]elete ","[C]ancel"};
	...
	_outtext("Disk information:");
	// 2004.08.03 huapage modify end

	//Add&Init Disk select window
	g_pDelDiskWin = new cwindow(PAGE_DELDISK, 5, 4, 11, 35);

	// 2004.08.03 huapage added begin
	// Add an "All Disk" for selection
	g_pDelDiskWin->additemNotSort("( ) All Disks",0);
	// 2004.08.03 huapage added end
	...
	{            
        ...
        g_pDelDiskWin->additemNotSort(szdrv,i+1); 
		// 2004.08.03 huapage modify end
	}
	...
	// 2004.08.03 huapage modify end
	{
		...
		g_deldiskinfo.nSelDisk = g_pDelDiskWin->m_pitemactive->btItem+DISKBASE-1;
		// 2004.08.03 huapage modify end
	}
...
#ifdef _DEBUG
	char szMsg[MAX_PATH];
	sprintf(szMsg,"SetDelDiskSel:%d",nSelItem);
	ShowMessage(szMsg);
#endif

	// 2004.08.03 huapage modify begin
	// while(g_pDelDiskWin->m_pitemactive->btItem != nSelItem)
	while( (g_pDelDiskWin->m_pitemactive->btItem -1 ) != nSelItem)
	// 2004.08.03 huapage modify end
	{
...
void showandsettardiskinfo()
{
...
#ifdef _DEBUG
	char szMsg[MAX_PATH];
	sprintf(szMsg,"showandsettardiskinfo, nSelDisk:%d",g_deldiskinfo.nSelDisk-DISKBASE+1);
	ShowMessage(szMsg);
#endif

	// 2004.08.03 added begin
	// if select "All Partitions" clear the content
	if( 0 == g_deldiskinfo.nSelDisk-DISKBASE+1)
	{
		g_pDelDiskSelWin->emptyitem();
   		g_pDelDiskSelWin->emptyscreen();
		return;
	}
	// 2004.08.03 added end
	...
	// 2004.08.03 modify for debug begin
	GetDriveParam(g_deldiskinfo.nSelDisk,&DriveParam);
	// GetDriveParam(g_deldiskinfo.nSelDisk-1,&DriveParam);
	// 2004.08.03 modify for debug end
	...
	// sprintf(szseldisk,DISKSELINFO,g_deldiskinfo.nSelDisk-DISKBASE,
	   		// DriveParam.dwCylinders,DriveParam.dwHeads,DriveParam.dwSecPerTrack);
	// 2004.08.03 modify for debug end
	...
	// g_diskviewinfo.nSelDisk				= g_deldiskinfo.nSelDisk-1;
	// 2004.08.03 modify for debug end

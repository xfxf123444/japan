...
BOOL deletecontent()
{
	...
	if(g_btCurFun == DELETEPARTITION)
	{
		...
		if( -1 == g_delparinfo.nselpar ) // means the user choose delete all partitions
		{
	        YG_PARTITION_INFO *pDiskInfo = g_pCurFixDiskInfo;

			while(pDiskInfo)
			{
				if(pDiskInfo->dwSystemFlag) // has dwSystemFlag means not free space
				{
					g_dDeletedSize = 0;
					g_delparinfo.nselpar ++;
					progressshowtarget();
					if (!DoDeleteSectors(pDiskInfo->dwStartSector,pDiskInfo->dwPartSize,pDiskInfo->btDiskNum + DISK_BASE ))
					{
						if (!g_bBeepOff)
						{
							Beep(1000,200);
	...
	else
	{
		...
		if( 0 == g_deldiskinfo.nSelDisk -DISKBASE+1 ) // means the user choose delete all disks
		{
			...
			for( nDiskIndex=0; nDiskIndex<g_nDiskNum; nDiskIndex++ )
			{
	            ...
	            GetDriveParam(nDiskIndex + DISKBASE,&DriveParam);

				g_deldiskinfo.nSelDisk = nDiskIndex + DISKBASE;
				...
				progressshowtarget();
			    if (!DoDeleteSectors(dwStart,DriveParam.dwSectorL, nDiskIndex + DISKBASE ))
				{
					if (!g_bBeepOff)
					{
						Beep(1000,200);
			...
			g_deldiskinfo.nSelDisk = DISKBASE-1;
		}
		else
		{
			// delete one disk

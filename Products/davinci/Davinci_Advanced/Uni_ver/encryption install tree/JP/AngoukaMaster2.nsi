...
OutFile "Setup.exe"
InstallDir "$PROGRAMFILES\FRONTLINE\埫崋壔儅僗僞乕2"
...
Section "MainSection" SEC01
  ...
  CreateDirectory "$SMPROGRAMS\埫崋壔儅僗僞乕2"
  CreateShortCut "$SMPROGRAMS\埫崋壔儅僗僞乕2\埫崋壔儅僗僞乕2.lnk" "$INSTDIR\FileEncrypt.exe"
  CreateShortCut "$SMPROGRAMS\埫崋壔儅僗僞乕2\僨僒僀僼傽乕201.lnk" "$INSTDIR\FileDecrypt.exe"
  CreateShortCut "$SMPROGRAMS\埫崋壔儅僗僞乕2\帺摦埫崋壔僼僅儖僟.lnk" "$INSTDIR\YGFSMon.exe"
  CreateShortCut "$SMPROGRAMS\埫崋壔儅僗僞乕2\Manual.lnk" "$INSTDIR\Manual.pdf"
...
Section -AdditionalIcons
  SetOutPath $INSTDIR
  ;CreateShortCut "$SMPROGRAMS\埫崋壔儅僗僞乕2\Uninstall.lnk" "$INSTDIR\uninst.exe"
SectionEnd
...
Section Uninstall
  ...
  SetShellVarContext All
  RMDir /r "$SMPROGRAMS\埫崋壔儅僗僞乕2"
  Delete "$SMSTARTUP\帺摦埫崋壔僼僅儖僟.lnk"
  Strcpy $R9 $WINDIR 2
  Strcpy $R9 "$R9\ProgramData\Microsoft\Windows\Start Menu\Programs\埫崋壔儅僗僞乕2"
  RMDir /r "$R9"
  SetOutPath $TEMP\eInspect
  SetOverwrite ifnewer

...
BOOL ParseCmdLine(WCHAR *pszCommand)
{
	...
	memset(g_szDir,0,MAX_PATH*MAX_PATH*sizeof(WCHAR));
	...
	if(p)
	{
		while(p) 
		{
			wcsncpy(g_szDir[i],pszCommand,p-pszCommand);
			p++;
			pszCommand = p;
			p = wcschr(pszCommand,L'*');
			i++;
	...
	else
	{
		...
		while(p)
		{
			*p = L'\0';
			...
			{
				*p = L':';
				if(L'"' == *pszCommand) pszCommand += 1;
				p = wcsrchr(pszCommand,L'"');
				if(p) *p = L'\0';
				wcscpy(g_szDir[i],pszCommand);
				i++;
				break;
			}
		}
...
BOOL GetSettingFileName(CString &strProgSettingDataPath)
{
	...
	strProgSettingDataPath=strProgSettingDir+"\\"+SETTING_DATA_FILE;
...
BOOL SavePassword( char *szUserName,char *szPassword,BOOL bAdmin )
{
	...
	hFile = CreateFile(szFile,GENERIC_READ|GENERIC_WRITE,FILE_SHARE_READ | FILE_SHARE_WRITE,
...
BOOL CheckPassword(char *szUserName,char *szPassword)
{
	...
	if (!stricmp(szUserName,"administrator"))
	{
		if (UserInfo[0].ulTryTimes >= 3)
		{
			GetLocalTime(&CurTime);
			if (CTime(CurTime) > CTime(UserInfo[0].LastTryTime))
			{
				TimeSpan = CTime(CurTime) - CTime(UserInfo[0].LastTryTime);
				if (TimeSpan.GetTotalMinutes() >= 10)
					UserInfo[0].ulTryTimes = 0;
			}
		}
		...
		{
			GetLocalTime(&UserInfo[0].LastTryTime);
//			YGAESDecryptData((UCHAR *)szPassword,strlen(szPassword),(UCHAR *)UserInfo[0].szUserName,(UCHAR *)UserInfo[0].szUserName,64);
			bResult = (!stricmp(szUserName,UserInfo[0].szUserName) && 
						!stricmp(szPassword,UserInfo[0].szUserPass));
			if (bResult)
			{
				UserInfo[0].ulLastTryStatus = 0;
				UserInfo[0].ulTryTimes = 0;
			}
			else
			{
				UserInfo[0].ulLastTryStatus = -1;
				UserInfo[0].ulTryTimes ++;
			}
//			YGAESEncryptData((UCHAR *)szPassword,strlen(szPassword),(UCHAR *)UserInfo[0].szUserName,(UCHAR *)UserInfo[0].szUserName,64);
			SetFilePointer(hFile,0,NULL,FILE_BEGIN);
			WriteFile(hFile,&UserInfo,sizeof(USERPASSSTRUCT)*2,&dwWrited,NULL);
		}
	}
	else
	{
		...
		if (UserInfo[1].ulTryTimes < 3)
		{
			GetLocalTime(&UserInfo[1].LastTryTime);
//			YGAESDecryptData((UCHAR *)szPassword,strlen(szPassword),(UCHAR *)UserInfo[1].szUserName,(UCHAR *)UserInfo[1].szUserName,64);
			bResult = (!stricmp(szUserName,UserInfo[1].szUserName) && 
						!stricmp(szPassword,UserInfo[1].szUserPass));
			if (bResult)
			{
				UserInfo[1].ulLastTryStatus = 0;
				UserInfo[1].ulTryTimes = 0;
			}
			else
			{
				UserInfo[1].ulLastTryStatus = -1;
				UserInfo[1].ulTryTimes ++;
			}
//			YGAESEncryptData((UCHAR *)szPassword,strlen(szPassword),(UCHAR *)UserInfo[1].szUserName,(UCHAR *)UserInfo[1].szUserName,64);
			SetFilePointer(hFile,0,NULL,FILE_BEGIN);
			WriteFile(hFile,&UserInfo,sizeof(USERPASSSTRUCT)*2,&dwWrited,NULL);
		}
	}

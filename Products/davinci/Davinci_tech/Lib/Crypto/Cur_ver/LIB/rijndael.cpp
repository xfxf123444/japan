...
#define ls_box(x)							 \
	((ULONG)sbx_tab[GETBYTE(x, 0)] <<	0) ^	\
	((ULONG)sbx_tab[GETBYTE(x, 1)] <<	8) ^	\
	((ULONG)sbx_tab[GETBYTE(x, 2)] << 16) ^	\
	((ULONG)sbx_tab[GETBYTE(x, 3)] << 24)

Rijndael::Rijndael(const UCHAR *userKey, unsigned int keylen)
...
#define f_rl(bo, bi, n, k)										\
	bo[n] = (ULONG)sbx_tab[GETBYTE(bi[n],0)] ^ 				   \
		rotlFixed(((ULONG)sbx_tab[GETBYTE(bi[(n + 1) & 3],1)]),  8) ^	\
		rotlFixed(((ULONG)sbx_tab[GETBYTE(bi[(n + 2) & 3],2)]), 16) ^	\
		rotlFixed(((ULONG)sbx_tab[GETBYTE(bi[(n + 3) & 3],3)]), 24) ^ *(k + n)
...
void RijndaelEncryption::ProcessBlock(const UCHAR *inBlock, UCHAR *outBlock) const
{
	ULONG b0[4], b1[4];
...
RijndaelDecryption::RijndaelDecryption(const UCHAR *userKey, unsigned int keylength)
...
#define i_rl(bo, bi, n, k)										\
	bo[n] = (ULONG)isb_tab[GETBYTE(bi[n],0)] ^ 				   \
		rotlFixed(((ULONG)isb_tab[GETBYTE(bi[(n + 3) & 3],1)]),  8) ^	\
		rotlFixed(((ULONG)isb_tab[GETBYTE(bi[(n + 2) & 3],2)]), 16) ^	\
		rotlFixed(((ULONG)isb_tab[GETBYTE(bi[(n + 1) & 3],3)]), 24) ^ *(k + n)

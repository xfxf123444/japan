...
void MD5Update (MD5_CTX *context,UCHAR *input,ULONG inputLen)
{
   ...
   */
  if (inputLen >= partLen) {
    YGECMemcpy((UCHAR *)&context->buffer[index], (UCHAR *)input, partLen);
  ...
  YGECMemcpy((UCHAR *)&context->buffer[index], (UCHAR *)&input[i],
     inputLen-i);
}
 ...
 */
void MD5Final(UCHAR digest[6],MD5_CTX * context)
{
  UCHAR bits[8];
  ULONG index, padLen;

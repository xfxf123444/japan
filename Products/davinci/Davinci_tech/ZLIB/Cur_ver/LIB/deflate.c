...
static int read_buf(z_streamp strm, BYTE FAR *buf, unsigned size);
...
#define CLEAR_HASH(s) \
    ...
    zmemzero((BYTE FAR *)s->head, (unsigned)(s->hash_size-1)*sizeof(*s->head));


/* ========================================================================= */
int deflateInit(strm, level, method, windowBits, memLevel, strategy,
		  version, stream_size)
    z_streamp strm;
    int  level;
    int  method;
    ...
    int stream_size;
{
    ...
    s = (deflate_state *)zcalloc(1, sizeof(deflate_state));
    ...
    s->window = (BYTE FAR *)zcalloc(s->w_size, 2*sizeof(BYTE));
    s->prev   = (Posf *)zcalloc(s->w_size, sizeof(Pos));
    s->head   = (Posf *)zcalloc(s->hash_size, sizeof(Pos));

    s->lit_bufsize = 1 << (memLevel + 6); /* 16K elements by default */

    overlay = (WORD FAR *)zcalloc(s->lit_bufsize, sizeof(WORD)+2);
    s->pending_buf = (BYTE FAR *) overlay;
    s->pending_buf_size = (DWORD)s->lit_bufsize * (sizeof(WORD)+2L);

    if (s->window == NULL || s->prev == NULL || s->head == NULL ||
        s->pending_buf == NULL) {
        strm->msg = (char*)z_errmsg[Z_NEED_DICT - Z_MEM_ERROR];
        deflateEnd (strm);
        return Z_MEM_ERROR;
    }
    ...
    int flush;
{
	...
	(s->status == FINISH_STATE && flush != Z_FINISH)) {
		strm->msg = (char*)z_errmsg[Z_NEED_DICT - Z_STREAM_ERROR];
		return Z_STREAM_ERROR;
    }
    if (strm->avail_out == 0)
	{
		strm->msg = (char*)z_errmsg[Z_NEED_DICT - Z_BUF_ERROR];
		return Z_BUF_ERROR;
	}
    ...
    z_streamp strm;
{
    ...
    if (strm->state->pending_buf)zcfree(strm->state->pending_buf);
    if (strm->state->head)zcfree(strm->state->head);
    if (strm->state->prev)zcfree(strm->state->prev);
    if (strm->state->window)zcfree(strm->state->window);
    ...
    deflate_state *s;
{
    ...
    do {
        more = (unsigned)(s->window_size -(DWORD)s->lookahead -(DWORD)s->strstart);

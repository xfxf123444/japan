...
PYG_LIST AllocMapEntryObj(PYG_LIST pObjList, DWORD dwSecPos, DWORD dwSectors)
{
    ...
    while (bSucc && GetImgSectorGroup(&GetGroup,dw1stSecPos,dwLastSecPos-dw1stSecPos,&Group))
    {
	if (!AllocateObjectList(Group.dwStartSec,Group.dwSectors,g_pSharkCache,g_pSharkData,sizeof(SECMAP),pObjList))
	    bSucc = FALSE;
    }
    */
    if (bSucc)
...
BOOL FlushMergeCacheObjs(PDISK_CACHE pCache, PBLKMOVER_DATA pBlkMoverData, DWORD dwSectors, PYG_LIST pFreeCacheList)
{
	...
	if (pObj->dwUseCnt == 0)
	{
		...
		if (pObj->DiskRange.dwSecAddr > 0 && pObj->DiskRange.dwSectors > 0)
		{
			...
			if (pCache == &g_SharkCache)
			{
				...
				SecureReadWriteSector(WRITE_OPERATION,
						      Group.dwStartSec,
						      Group.dwSectors,
						      &pCache->pCache[dwSecOfs*SECTOR_SIZE],
						      pBlkMoverData);
				dwSecOfs += Group.dwSectors;
			    }
			    */
		...
		pObj->bDirty = FALSE;
		// SecureReadWriteSector may have yield control and cause pObj to be taken by another process.
